...
public:
	...
	int add(const T &key, int indice);
...
template<typename T>
BPTreeNode<T>::BPTreeNode(int degree, bool isLeaf) : degree(degree), isLeaf(isLeaf), cnt(0), parent(nullptr), sibling(nullptr)
{
	children.resize(degree + 1);
	keys.resize(degree);
	indices.resize(degree);
}
...
BPTreeNode<T> *BPTreeNode<T>::split(T &key)
{
	...
	if (isLeaf)
	{
		...
		for (int i = minimal + 1; i < degree; i++)
		{
			...
			newNode->indices[i - minimal - 1] = indices[i];
		}
...
template<typename T>
int BPTreeNode<T>::add(const T &key, int indice)
{
	int index;
	bool keyExists = search(key, index);
	if (keyExists)
...
public:
	...
	bool insert(const T &key, int indice);

	bool remove(const T &key, const T &last);
...
int BPTree<T>::find(const T &key)
{
	...
	res.node = NULL;
	if (!root) { return -1; }
	if (findKeyFromNode(root, key, res)) { return res.node->indices[res.index]; }
	else { return -1; }
}
...
template<typename T>
bool BPTree<T>::insert(const T &key, int indice)
{
	NodeSearchParse<T> res;
	if (!root) { initBPTree(); }
...
template<typename T>
bool BPTree<T>::remove(const T &key, const T &last)
{
	NodeSearchParse<T> Last;
	int goingdelete = find(key);
	Last = findNode(last);
	Last.node->indices[Last.index] = goingdelete;
...
bool BPTree<T>::deleteLeafLL(BPTree::TreeNode node, BPTree::TreeNode parent, BPTree::TreeNode sibling, int index)
{
	...
	{
		node->keys[i] = node->keys[i - 1];
		node->indices[i] = node->indices[i - 1];
	}
	node->keys[0] = sibling->keys[sibling->cnt - 1];
	node->indices[0] = sibling->indices[sibling->cnt - 1];
	sibling->removeAt(sibling->cnt - 1);
...
bool BPTree<T>::deleteLeafLR(BPTree::TreeNode node, BPTree::TreeNode parent, BPTree::TreeNode sibling, int index)
{
	...
	for (int i = 0; i < node->cnt; i++)
	{
		...
		sibling->indices[i + sibling->cnt] = node->indices[i];
	}
	sibling->cnt += node->cnt;
...
bool BPTree<T>::deleteLeafRL(BPTree::TreeNode node, BPTree::TreeNode parent, BPTree::TreeNode sibling, int index)
{
	...
	node->indices[node->cnt] = sibling->indices[0];
	node->cnt++;
	sibling->removeAt(0);
...
bool BPTree<T>::deleteLeafRR(BPTree::TreeNode node, BPTree::TreeNode parent, BPTree::TreeNode sibling, int index)
{
	...
	{
		node->keys[node->cnt + i] = sibling->keys[i];
		node->indices[node->cnt + i] = sibling->indices[i];
	}
	if (node == parent->children[0])
	{
